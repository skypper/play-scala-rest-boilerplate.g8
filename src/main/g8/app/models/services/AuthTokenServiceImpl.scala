...
class AuthTokenServiceImpl @Inject() (authTokenDAO: AuthTokenDAO)(
  ...
  def create(userID: UUID, expiry: FiniteDuration = 5 minutes) = {
    val token = AuthToken(UUID.randomUUID(), userID, ZonedDateTime.now().plusSeconds(expiry.toSeconds.toInt))
   ...
   */
  def clean = authTokenDAO.findExpired(ZonedDateTime.now()).flatMap { tokens =>
